...
Run `uv run pytest -q --tb=short --no-header $ARGUMENTS 2>&1 | tail -40` in the project root.

If no arguments are provided, run all tests: `uv run pytest -q --tb=short --no-header 2>&1 | tail -40`.
